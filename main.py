...
class Camera_Database:
  ...
  def add_camera(self, camera_name, sensor_width_px, sensor_height_px, pixel_size_nm, focal_length_mm):
    ...
    Args:
      ...
      pixel_size_nm (float): The size of each pixel in nanometers.
  ...
  def calculate_gsd(altitude_m, sensor_width_px, pixel_size_nm, focal_length_mm):
    """
        ...
        Args:
            ...
            pixel_size_nm (float): The sensor pixel size in nanometers.
            focal_length_mm (float): The sensor focal length in millimeters.

        Returns:
            float: The calculated GSD in centimeters.
        """
    pixel_size_m = pixel_size_nm * NM_TO_M   # Convert nanometre to meter
...
def add_cam(camera_database):  
  ...
  is_valid = False
  while is_valid == False: 
    pixel_size_nm = input("Enter pixel size in nanometers [B for back]: ")
    if pixel_size_nm == "B":
      display_welcome_message()
      display_menu(camera_database)
      return
    is_valid = validate_input(pixel_size_nm, min_value=1, max_value=100000, expected_types=[int,float])
    if is_valid:
  ...
  while is_valid == False: 
    ...
    if focal_length_mm  == "B":
      display_welcome_message()
      display_menu(camera_database)
      return
    is_valid = validate_input(focal_length_mm, min_value=1, max_value=1000, expected_types=[int,float])
    if is_valid:
  ...
  while True:
    ...
    # Promt for new altitude or to go back
      ...
      if altitude_m.isdigit():
        ...
        if not validate_input(altitude_m, min_value=0, max_value=10000, 
